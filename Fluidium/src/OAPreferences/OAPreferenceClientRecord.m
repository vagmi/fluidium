...
- (NSImage *)iconImage;
{
    ...
    if ([iconName hasPrefix:OAPreferenceClientRecordIconNameAppPrefix]) {
        ...
        NSBundle *bundle = [NSBundle bundleForClass:NSClassFromString(cls)];
        //        iconImage = [[NSImage imageNamed:iconName inBundle:bundle] retain];
        NSString *path = [bundle pathForImageResource:iconName];
        if ([path length]) {
            NSURL *URL = [NSURL fileURLWithPath:path];
            iconImage = [[NSImage alloc] initWithContentsOfURL:URL];
        } else {
            iconImage = [[NSImage imageNamed:iconName] retain];
        }
    }
...
{
    NSBundle *bundle = [NSBundle bundleForClass:[OAPreferenceClientRecord class]];
    [controller setTitle:[NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"Loading %@...", @"OmniAppKit", bundle, "preference bundle loading message format"), title]];
