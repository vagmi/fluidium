...
- (void)_defaultsDidChange:(NSNotification *)notification;
{
    if ([window isVisible]) {
        ...
        [self performSelector:@selector(validateRestoreDefaultsButton) withObject:nil afterDelay:0];
...
+ (NSString *)_localizedCategoryNameForCategoryName:(NSString *)categoryName;
{
    id value = [LocalizedCategoryNames objectForKey:categoryName];
    if (!value)
        value = categoryName;
...
+ (void)_registerClassName:(NSString *)className inCategoryNamed:(NSString *)categoryName description:(NSDictionary *)description;
{
    ...
    NSBundle *classBundle = [NSBundle bundleForClass:NSClassFromString(className)]; //[OFBundledClass bundleForClassNamed:className];
...
//    }
    ...
    if (identifier == nil) {
        ...
        identifier = [description objectForKey:@"shortTitle"];
        if (!identifier) {
            identifier = titleEnglish;
        }
    }
    ...
    BOOL res = NO;
    id value = [description objectForKey:@"hidden"]; //[self objectForKey:@"hidden"];
